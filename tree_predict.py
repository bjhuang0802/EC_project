...
timew=['5-OCT-16','6-OCT-16','7-OCT-16','8-OCT-16','9-OCT-16','10-OCT-16','11-OCT-16','12-OCT-16']
x0=[]
y0=[]
f=[]
pattern = '%d-%b-%y'
#rawdata = pd.read_csv('rawdata020/dataset'+timew[-1]+'.csv',dtype={'gid':str,'event':str,'productId':str})
rawdata = pd.read_csv('rawdata020/dataset020.csv',dtype={'fid':str,'event':str,'cur_cam':str,'productId':str})
...
print 'The predict-buy module using decision tree.'
...
print >>fp1,'The predict-buy module using decision tree.'
...
for fig_num, kernel in enumerate(('linear','rbf')):
    ...
    joblib.dump(clf, 'model/tree/tree_predict_'+name+'rate'+str(rate)+'-'+str(nday)+'.pkl') 

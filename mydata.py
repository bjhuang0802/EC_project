...
def load(rawdata,rate,nw_x,nw_y,timew):
    ...
    raw1 = csv.reader(open('rawdata020/session_sort.csv'))
    ...
    for row in data.iterrows():
        ...
        if row[1]['cur_cam'] != '0' and (row[1]['time'] < purT[row[1]['fid']]):
            ufeat[idx][2] +=1
        if row[1]['purchase'] == 2 and (row[1]['time'] < purT[row[1]['fid']]-secs_a_day):
            ufeat[idx][3] += 1
        #if row[1]['purchase'] == 2 and (row[1]['time'] >purT[row[1]['fid']]-60) and (row[1]['time'] < purT[row[1]['fid']]+10): 
        if row[1]['purchase'] == 2 and (row[1]['time'] > maxct - secs_a_day): 
